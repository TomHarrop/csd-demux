...
np_barcodes_file <- "data/nanopore_barcode_positions.csv"
indiv_barcodes_file <- "data/pool1_amounts.csv"

vcf_file <- "output/060_freebayes/variants_filtered.vcf"
gds_file <- tempfile(fileext = ".gds")

# work out which indiv was which barcode
indiv_barcodes <- fread(indiv_barcodes_file)
np_barcodes <- fread(np_barcodes_file)

indiv_to_bc_name <- merge(
    indiv_barcodes[, .(sample_id, bc = paste0(bc_row, bc_col))],
    np_barcodes[
        , .(barcode_name = BC,
            bc = paste0(row, col))],
    by = "bc")[, .(sample_id, barcode_name)]

map_vector <- indiv_to_bc_name[, structure(sample_id, names = barcode_name)]
...
colnames(ibs_df) <- plyr::revalue(colnames(ibs_df), map_vector)
rownames(ibs_df) <- plyr::revalue(rownames(ibs_df), map_vector)
